...
class Status:
    def print(self):
        print(f"{self.msg} {round(self.cur*100/self.max)}")


class QuanshengUVK5Radio(Serial):
    BLOCK_SIZE = 80
    ...
    def sync_in(self):
        ...
        if not self.FIRMWARE_VERSION:
            sys.exit('Unable to determine firmware version')
    ...
    def get_version(self):
        ...
        sys.exit("Failed to initialize radio{}".format(ERROR_TIP))
    ...
    def writemem(self, data, addr):
        ...
        if (o[0] == 0x1e and o[4] == (addr & 0xff) and o[5] == (addr >> 8) & 0xff):
            ...
            sys.exit("Bad response to writemem{}".format(ERROR_TIP))
    ...
    def _send_command(self, data: bytes):
        ...
        try:
            ...
            sys.exit("Error writing data to radio{}".format(ERROR_TIP))
        return result
    ...
    def _receive_reply(self):
        header = self.read(4)
        if len(header) != 4:
            sys.exit("Header short read{}".format(ERROR_TIP))
        if header[0] != 0xAB or header[1] != 0xCD or header[3] != 0x00:
            sys.exit("Bad response header{}".format(ERROR_TIP))

        cmd = self.read(int(header[2]))
        if len(cmd) != int(header[2]):
            sys.exit("Command body short read{}".format(ERROR_TIP))

        footer = self.read(4)

        if len(footer) != 4:
            sys.exit("Footer short read{}".format(ERROR_TIP))

        if footer[2] != 0xDC or footer[3] != 0xBA:
            sys.exit("Bad response footer{}".format(ERROR_TIP))
...
if __name__ == '__main__':
    sp1 = 38400
    sp2 = 115200
    k5 = QuanshengUVK5Radio('/dev/ttyUSB0', sp1)
    k5.check()
